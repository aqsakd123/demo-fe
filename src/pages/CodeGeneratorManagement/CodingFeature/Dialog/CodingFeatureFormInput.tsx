...
import { CodingFeatureInput } from '../CodingFeatureList'
...
export type FormProps = {
  id: string
  language: string
  defaultValues?: CodingFeatureInput
  onSubmit: ((data: any) => void) | ((data: any) => Promise<void>)
}

const CodingFeatureFormInput: React.FC<FormProps> = (props: FormProps) => {
  ...
  useEffect(() => {
    dispatch(codingFeatureStore.actions.setDirty(isDirty))
  }, [isDirty])

  const handleSubmitForm = handleSubmit(async (data: CodingFeatureInput) => {
    const submitData: CodingFeatureInput = {
      ...data,
...
export default CodingFeatureFormInput
