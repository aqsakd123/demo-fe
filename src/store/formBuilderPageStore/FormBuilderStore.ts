import { DataFormType } from '@app/pages/formBuilderPages/Dialog/DataFormInput'
import { PayloadAction, createSlice } from '@reduxjs/toolkit'

export type DialogState = 'edit' | 'add' | 'none'

export type FormBuilderState = {
  formDataList: DataFormType[]
  formDialogStatus: DialogState
  formEditData?: DataFormType
  isDirty: boolean
}
...
const formBuilderStore = createSlice({
  ...
  reducers: {
    setDataFormList: (state: FormBuilderState, action: PayloadAction<DataFormType[]>) => {
      state.formDataList = action.payload
    },
    setDataFormEdit: (state: FormBuilderState, action: PayloadAction<DataFormType>) => {
